...
T popl(priority_queue<T, Container, Compare> &que) {
    T a = que.top();
    que.pop();
    return a;
}
...
template <typename T, typename Container, typename Compare>
T peekl(priority_queue<T, Container, Compare> &que) {
    return que.top();
}
template <typename T>
T peekl(stack<T> &st) {
...
template <typename T>
concept ListOrVector =
    is_same_v<T, list<typename T::value_type>> or is_same_v<T, vector<typename T::value_type>>;
...
#define EACHA(...) \
    OVERLOAD8(__VA_ARGS__, EACHZ6, EACHZ5, EACHZ4, EACHZ3, EACHZ2, EACHZ1)(__VA_ARGS__)
#define EACHB(...) \
    OVERLOAD8(__VA_ARGS__, EACHB6, EACHB5, EACHB4, EACHB3, EACHB2, EACHB1)(__VA_ARGS__)
...
#define pe(...)         \
    print(__VA_ARGS__); \
    PR1(::std::endl)  // print endl
#define OVERLOAD20(_1, _2, _3, _4, _5, _6, _7, _8, _9, _10, _11, _12, _13, _14, _15, _16, _17, \
                   _18, _19, NAME, ...)                                                        \
    NAME
#define FUNA0(...)
#define FUNA1(T, x) T
...
#define FUNA(...)                                                                                 \
    OVERLOAD20(__VA_ARGS__, FUNA9, FUNA9, FUNA8, FUNA8, FUNA7, FUNA7, FUNA6, FUNA6, FUNA5, FUNA5, \
               FUNA4, FUNA4, FUNA3, FUNA3, FUNA2, FUNA2, FUNA1, FUNA1, FUNA0, FUNA0)              \
    (__VA_ARGS__)
#define FUNB(...)                                                                                 \
    OVERLOAD20(__VA_ARGS__, FUNB9, FUNB9, FUNB8, FUNB8, FUNB7, FUNB7, FUNB6, FUNB6, FUNB5, FUNB5, \
               FUNB4, FUNB4, FUNB3, FUNB3, FUNB2, FUNB2, FUNB1, FUNB1, FUNB0, FUNB0)              \
    (__VA_ARGS__)
...
int main() {
    ...
    // pr(tt);
    fk(tt) {
        int n = ran.mtrand(1, 11);
        int m = ran.mtrand(1, 1001);
        const int C = 100;
        ...
        vpii edges;
        fk (i, 1, n) {
            int u = ran.mtrand(0, i);
            edges.emplace_back(u + 1, i + 1);
            if (ran.mtrand(2)) {
                swap(edges.bk().fi, edges.bk().se);
            }
        }
        ::std::shuffle(all(edges), mt19937(random_device()()));
        ...
        while (m--) {
            int tp = ran.mtrand(0, 2);
            if (tp == 0) {
                int a = ran.mtrand(1, n + 1);
                int b = ran.mtrand(1, n + 1);
                int color = ran.mtrand(1, 101);
                pr('C', a, b, color);
            } else {
                int a = ran.mtrand(1, n + 1);
                int b = ran.mtrand(1, n + 1);
